  ...
  for i = g.rooms[rn].top_row + 1, g.rooms[rn].bottom_row - 1 do
    for j = g.rooms[rn].left_col + 1, g.rooms[rn].right_col - 1 do
      if g.blind > 0 then
        ...
        if
          not (g.dungeon[i][j][g.OBJECT] or g.dungeon[i][j][g.STAIRS])
          and not (g.detect_monster and g.dungeon[i][j][g.MONSTER])
        then
...
function g.get_dungeon_char(row, col)
  ...
  then
    ...
    if mask[g.DOOR] then
      if mask[g.HIDDEN] then
        if
          ((col > 0) and g.dungeon[row][col - 1][g.HORWALL])
          or ((col < g.DCOLS - 1) and g.dungeon[row][col + 1][g.HORWALL])
        then
          return "-"
        else
          return "|"
...
function g.gr_row_col(mask)
  ...
  until not (
      (rn == g.NO_ROOM)
      or (not check_mask(g.dungeon[r][c], mask))
      or (check_mask(g.dungeon[r][c], mask, true))
      or not (g.rooms[rn].is_room == g.R_ROOM or g.rooms[rn].is_room == g.R_MAZE)
      or (r == g.rogue.row and c == g.rogue.col)
    )
...
function g.get_room_number(row, col)
  for i = 0, g.MAXROOMS - 1 do
    if
      (row >= g.rooms[i].top_row and row <= g.rooms[i].bottom_row)
      and (col >= g.rooms[i].left_col and col <= g.rooms[i].right_col)
    then
      return i
    end
  end
