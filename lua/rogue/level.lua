...
local function make_maze(r, c, tr, br, lc, rc)
  ...
  for i = 0, 3 do
    if dirs[i] == g.UPWARD then
      if
        ((r - 1) >= tr)
        and not g.dungeon[r - 1][c][g.TUNNEL]
        and not g.dungeon[r - 1][c - 1][g.TUNNEL]
        and not g.dungeon[r - 1][c + 1][g.TUNNEL]
        and not g.dungeon[r - 2][c][g.TUNNEL]
      then
    ...
    elseif dirs[i] == g.LEFT then
      if
        ((c - 1) >= lc)
        and (c - 2 >= 0) -- fixed original bug: access dungeon[r][-1]
        and not g.dungeon[r][c - 1][g.TUNNEL]
        and not g.dungeon[r - 1][c - 1][g.TUNNEL]
        and not g.dungeon[r + 1][c - 1][g.TUNNEL]
        and not g.dungeon[r][c - 2][g.TUNNEL]
      then
