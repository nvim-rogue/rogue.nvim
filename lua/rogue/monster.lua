...
function g.mv_mons()
  ...
  while monster do
    ...
    if not goto_NM_flag then
      local flew = false
      if
        monster.m_flags[g.FLIES]
        and not monster.m_flags[g.NAPPING]
...
function g.move_mon_to(monster, row, col)
  ...
  if
    ...
    and g.get_room_number(row, col) ~= g.cur_room
    and g.dungeon_equals(g.dungeon[mrow][mcol], g.FLOOR)
    and g.blind == 0
  then
    g.mvaddch(mrow, mcol, " ")
  end
...
function g.mon_can_go(monster, row, col)
  ...
  then
    if
      ...
      or (monster.row > g.rogue.row and row > monster.row)
      or (monster.col < g.rogue.col and col < monster.col)
      or (monster.col > g.rogue.col and col > monster.col)
    then
      return false
    end
...
function g.create_monster()
  ...
  for i = 0, 8 do
    ...
    col = c
    if
      (row == g.rogue.row and col == g.rogue.col)
      or (row < g.MIN_ROW)
      or (row > (g.DROWS - 2))
      or (col < 0)
      or (col > (g.DCOLS - 1))
