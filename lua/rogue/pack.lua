  ...
  while op do
    if (op.what_is == obj.what_is) and (op.which_kind == obj.which_kind) then
      if
        ...
        or (
          (obj.what_is == g.WEAPON)
          and ((obj.which_kind == g.ARROW) or (obj.which_kind == g.DAGGER) or (obj.which_kind == g.DART) or (obj.which_kind == g.SHURIKEN))
          and (obj.quiver == op.quiver)
        )
      then
...
function g.drop()
  if
    g.dungeon[g.rogue.row][g.rogue.col][g.OBJECT]
...
function g.pack_count(new_obj)
  ...
  while obj do
    ...
    elseif not new_obj then
      count = count + 1
    elseif
      (new_obj.what_is ~= g.WEAPON)
      or ((obj.which_kind ~= g.ARROW) and (obj.which_kind ~= g.DAGGER) and (obj.which_kind ~= g.DART) and (obj.which_kind ~= g.SHURIKEN))
      or (new_obj.which_kind ~= obj.which_kind)
      or (obj.quiver ~= new_obj.quiver)
    then
      count = count + 1
    end
    obj = obj.next_object
